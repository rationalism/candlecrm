...
(defn cypher-labels [num labels]
  (str "(a" num ":" (str/join ":" labels) ")"))

(defn insert-query [num s]
  (str "CREATE " s " RETURN "
       (->> num range (interleave (repeat num "a"))
            (partition 2) (map str/join)
            (str/join ","))))

(defn multiple-insert! [labels num]
  (->> (range num)
       (map #(cypher-labels % labels))
       (str/join ",")
       (insert-query num)
       cypher-query first vals))

(defn create-merge [item-pair]
  (cond
    (-> item-pair first empty?) []
    (-> item-pair first count (= 1))
    (if (-> item-pair first vals first coll?)
      (->> item-pair first vals first
           (map #(into {} [[(-> item-pair first keys first) %]]))
           (map vector) (map #(conj % (second item-pair)))
           (map create-merge))
      (str "MATCH (root) WHERE ID(root) = " (second item-pair)
           " CREATE UNIQUE (root)-[:"
           (-> item-pair first keys first cypher-esc-token)
           "]->(v:" s/val-node " {"
           (-> item-pair first first cypher-property)
           "})"))
    :else
    (->> item-pair first (partition 1)
         (map #(into {} %))
         (map #(create-merge [% (second item-pair)]))
         flatten)))

(defn batch-create! [items labels]
  (->> items count (multiple-insert! labels)
       (map :id) (interleave items) (partition 2)
       (map create-merge)
       (pmap cypher-combined-tx)))
